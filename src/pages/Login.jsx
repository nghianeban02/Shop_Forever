...
const Login = () => {
  ...
  return (
   ...
   {currentState==='Login'?'':<input className='w-full px-3 py-2 border border-gray-800' placeholder='Name' type="text" />} 
    ...
    <div className='w-full flex justify-between text-sm mt-[-8px]'>
      <p className='cursor-pointer'>
        ...
        {
          currentState==='Login'
          ? <p onClick={()=>setCurrentState('Sign up')} className='cursor-pointer'> Create account
         </p>
         : <p onClick={()=>setCurrentState('Login')} className='cursor-pointer'> Login Here        
          </p>
        }
    </div>
